...
/// A bridge for Swift types whose Java wrapper classes are generated by jextract.
///
...
extension JextractedTypeBridge {
  ...
  public static func fromJavaObject(_ obj: jobject?, in environment: JNIEnvironment) -> SwiftType {
    guard let obj else {
      fatalError("\(Self.self).fromJavaObject received a null Java object")
    }
    ...
    guard let valuePointer$ = UnsafeMutablePointer<SwiftType>(bitPattern: selfPointerBits$) else {
      fatalError("\(Self.self).fromJavaObject received a null Swift memory address")
    }
